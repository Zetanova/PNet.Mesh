...
**Benchmarks:** restore/build in Release, then run BenchmarkDotNet with `dotnet run --project src/PNet.Mesh.Benchmarks/PNet.Mesh.Benchmarks.csproj -c Release -- --filter '*'`; macro JSON harnesses use `--macro in-memory` and `--macro udp-loopback`; privileged TUN topology uses `--tun-topology plan|preflight|create|teardown`; current baseline/policy live in `.agents/docs/benchmarks/`.
