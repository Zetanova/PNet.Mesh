...
|-------|--------|--------|-------|-------|
| Multi-node mesh topology | `Legacy artifacts: docker-compose.yml, docker-compose.e2e.yml` | Covered | [003](issues/003-mesh-topology-e2e-story.md) | Historical six-node topology kept for #011; `scripts/e2e-mesh-topology.sh` remains a legacy compose smoke artifact, while supported mesh coverage lives in `src/PNet.Mesh.E2ETests/PNet.Mesh.E2ETests.csproj` through named methods such as `PNet.Actor.E2ETests.Mesh.PNetMeshTestNodeHarnessTests.direct_peers_exchange_payloads_in_both_directions` and `PNet.Actor.E2ETests.Mesh.PNetMeshTestNodeHarnessTests.multi_hop_route_crosses_separated_container_segments`. |
