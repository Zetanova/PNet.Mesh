...
dotnet run --project src/PNet.Mesh.Benchmarks/PNet.Mesh.Benchmarks.csproj -c Release -- --macro raw-secureframe --payload 128 --warmup 00:00:05 --duration 00:00:30
...
dotnet run --project src/PNet.Mesh.Benchmarks/PNet.Mesh.Benchmarks.csproj -c Release -- --tun-benchmark pnet-mesh-tun --ping-count 1 --iperf-duration 3s --trace-output-dir artifacts/benchmarks/packet-trace
...
Macro benchmarks run through the same Release-only benchmark executable with `--macro`. The `raw-secureframe` scenario measures fixed-duration raw secure-frame encrypt/decrypt/dispatch throughput without protobuf, sockets, or session logic. The `in-memory` scenario measures two-session payload exchange without sockets or Docker. The `udp-loopback` scenario measures encrypted packet exchange over localhost UDP. These scenarios emit JSON with packets/sec, payload bytes/sec, wire bytes/sec, p50/p95/p99 latency, allocated bytes, GC counts, runtime, OS, CPU architecture, and payload size. `--udp-socket-probe` compares UDP receive APIs (`saea`, task-based, and blocking modes) over loopback and emits JSON latency percentiles. `--macro testnode-smoke` documents the optional manual TestNode/container perf smoke as non-deterministic; it is not run by default.

Privileged Linux TUN benchmark topology is manual and report-only. `--tun-topology plan` emits the fixed dual-stack Docker topology, `--tun-topology preflight` reports `pass`, `skip`, or `fail`, and `--tun-topology create`/`teardown` manages only labeled Docker resources for PNet.Mesh.Tun and `wireguard-go` benchmark scenarios. `--tun-benchmark pnet-mesh-tun` and `--tun-benchmark wireguard-go` create that topology, attempt IPv4/IPv6 ping and `iperf3`, emit parsed JSON success/failure details plus one representative process metric record, and tear the topology down. PNet.Mesh.Tun runs accept `--iperf-bytes <bytes>` for fixed-size load, `--iperf-bandwidth <rate>` for the UDP send cap, `--iperf-window <size>` for the UDP socket window passed to `iperf3 -w`, `--managed-heap-growth-limit-bytes <bytes>` for before/after managed-heap validation, `--trace-output-dir <dir>` for packet-trace CSV capture, plus `--pnet-udp-receive-mode async|blocking` and `--pnet-udp-socket-buffer-bytes <bytes>` for receive-path probes. `--tun-bandwidth-series` defaults to PNet.Mesh.Tun to PNet.Mesh.Tun and `--tun-bandwidth-series --scenario wireguard-go` runs the same TUN-to-`wireguard-go`-to-`wireguard-go`-to-TUN series at 10M, 25M, 50M, 100M, 250M, 500M, 1G, and 2.5G caps with 100 MB per `iperf3` transfer. PNet.Mesh.Tun series runs include managed-heap validation after each cap; `wireguard-go` series runs record process CPU/RSS and mark .NET managed counters not applicable. `--tun-compare --pnet <pnet-json> --wireguard <wireguard-json>` reads saved benchmark JSON and emits one comparison schema with raw traffic/command data plus normalized latency, bandwidth, packet loss, single-process RSS/HWM, CPU, thread, settings, payload mode, iperf bandwidth/datagram size, environment, implementation, traceability, and managed-runtime fields. `scripts/bench-tun-comparison.sh` is the single-command wrapper for build, preflight, both scenarios, comparison, deterministic artifacts, optional report-only `--baseline <comparison.json>` regression output, and cleanup. The manual/scheduled workflow and artifact layout live in `.agents/docs/benchmarks/tun-workflow.md`.
